...
public class UIUpdateController : MonoBehaviour
{
    ...
    public IEnumerator DisplayResourceLoss(ResourceEnum resourceType, int quantity) {
        ...
        gameControllerScript.uiResourcesLossTextMap[resourceType].alpha = 1.0f;
        gameControllerScript.uiResourcesLossTextMap[resourceType].text = "-" + quantity;

        //Get origin and destination position for "bounce"
        float timeEllapsed = 0;
        var origin = gameControllerScript.uiResourcesLossTextMap[resourceType].gameObject.transform.position;
        var destination = origin + Constants.RESOURCE_LOSS_DISPLACE;

        //Move text up
        while (timeEllapsed < Constants.RESOURCE_LOSS_MOVEMENT_TIME) {
            transform.position = Vector3.Lerp(origin, destination, timeEllapsed / Constants.RESOURCE_LOSS_MOVEMENT_TIME);
        ...
        //Fade out text
        timeEllapsed = 0;
        gameControllerScript.uiResourcesLossTextMap[resourceType].CrossFadeAlpha(0.0f, Constants.RESOURCE_LOSS_MOVEMENT_TIME, true);
        
        //Move text down
        while (timeEllapsed < Constants.RESOURCE_LOSS_MOVEMENT_TIME) {
            transform.position = Vector3.Lerp(destination, origin, timeEllapsed / Constants.RESOURCE_LOSS_MOVEMENT_TIME);
            timeEllapsed += Constants.RESOURCE_LOSS_MOVEMENT_LERP_TIME;
            yield return new WaitForSeconds(Constants.RESOURCE_LOSS_MOVEMENT_LERP_TIME);
        }
    }
    ...
    public void ConsumeResources() {
        ...
        foreach (ResourceEnum resource in Enum.GetValues(typeof(ResourceEnum))) {
            var resourceLoss =
                gameControllerScript.mainBuildingList.Count * Constants.MAIN_BUILDING_MANTAINING_COST[resource] +
                gameControllerScript.oreGatherersList.Count * Constants.ORE_GATHERER_COST[resource];
